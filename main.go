...
func newGomokuApp() *gomokuApp {
	...
	w.Resize(fyne.NewSize(1240, 920))
...
func (g *gomokuApp) buildUI() {
	...
	subtitle := widget.NewLabel("Play against the original alpha-beta search bot on a 15x15 board.")
	...
	sidebar := container.NewVBox(
		...
		widget.NewCard("Opponent", "", widget.NewLabel(
			"Opponent: Alpha-Beta\nSearch depth: 3\nBoard size: 15x15",
		)),
	)

	boardCard := widget.NewCard("Board", "Coordinates make it easier to track lines and threats.", container.NewPadded(boardGrid))
...
func (g *gomokuApp) playHumanMove(row, col int) {
	...
	g.status = fmt.Sprintf("%s is thinking...", aiName)
	g.refreshBoard()

	move := g.board.BestMove(g.aiPlayer, searchDepth)
	if move.Row == -1 || move.Col == -1 {
		g.status = fmt.Sprintf("%s could not find a valid move.", aiName)
		g.gameOver = true
		g.refreshBoard()
		return
	}
	if err := g.board.Place(move.Row, move.Col, g.aiPlayer); err != nil {
		g.status = fmt.Sprintf("%s move failed: %v", aiName, err)
		g.gameOver = true
		g.refreshBoard()
		return
	}
	...
	if g.board.HasFive(move.Row, move.Col, g.aiPlayer) {
		g.status = fmt.Sprintf("%s (%s) wins!", aiName, g.aiPlayer.String())
		g.gameOver = true
		g.refreshBoard()
		return
	}
	...
	g.status = fmt.Sprintf("%s played %d, %d. Your turn.", aiName, move.Row, move.Col)
	g.refreshBoard()
}
...
func (g *gomokuApp) refreshBoard() {
	for row := 0; row < boardSize; row++ {
		for col := 0; col < boardSize; col++ {
			...
			if cell == game.Empty && !g.gameOver && g.current == g.humanPlayer {
				button.Enable()
...
func (g *gomokuApp) refreshMeta() {
	g.infoLabel.SetText(fmt.Sprintf("Board: %dx%d   Alpha-Beta depth: %d", boardSize, boardSize, searchDepth))
}
